...
type Dedup struct{ minTokens int }
...
func newDedup(raw []byte) (components.Component, error) {
	...
	return &Dedup{minTokens: cfg.MinTokens}, nil
}
...
func (d *Dedup) Offload(req *schemas.BifrostChatRequest, rep *components.Report, c *components.Ctx) ([]string, error) {
	...
	for i := range req.Input {
		...
		// Later duplicate: stash + collapse to a pointer.
		c.Store.Put(h, []byte(content))
		schema.SetMessageText(m, "[identical to an earlier tool output] "+expand.Marker(h))
		keys = append(keys, h)
	}
	if len(keys) == 0 {
		rep.Skipped = true
